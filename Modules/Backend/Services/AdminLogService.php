...
class AdminLogService
{
    ...
    public function LogAdd($params){
        ...
        if($model == 'adminLogin'){
            ...
            $log_data['operate_target'] = "用户登陆";
        }else{
            // 处理公共操作
            $temp_operate_target = $this->commonLogs($controller."/".$model);
            if($temp_operate_target){
                $log_data['operate_target'] = $temp_operate_target;
            }else{
        ...
        if(!isset($log_data['operate_target'])){
    ...
    // 公共操作
    protected function commonLogs($data){
        $common = [
            'AdminController/adminChangePassword' => '修改密码',
            'ReportController@receiptRead' => '编辑回执为已读',
            'ReportController@downloadRecord' => '下载文件',
            'ClientInfoController/clientInfoCountNew' => '业主情况/业主统计',  //临时新接口
        ];
        return isset($common[$data])?$common[$data]:false;
    }
